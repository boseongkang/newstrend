...
STOP_WORDS = {
    ...
    # ── Round-4+5: confirmed noise from ticker analysis ──
    "analysts","analyst","governor","shows","show","coming","found",
    "called","calls","play","plays","winner","winners","streaming",
    "highlights","highlight","professional","corporate","silver",
    "states","state","cuts","amid","access","leaders","leader",
    "videos","video","congress","special","money","house",
    "hired","named","joined","filing","filed","quarter","annual",
    "fiscal","revenue","revenues","profit","profits","margin","margins",
    "higher","lower","record","records","posted","posts",
    "close","closed","opened","right","long","short","early","late",
    "week","months","years","days","hours","times",
    "wall","back","just","also","like","well","even",
    "america","india","part","general","focus","deployment","accused",
    "announces","announce","plans","plan","videos","video","claims","claim",
...
def main():
    ...
    # *_tokens.csv, *_tokens.jsonl, 그리고 YYYY-MM-DD.csv / YYYY-MM-DD.jsonl
    import re as _re
    def _is_date_file(p):
        return bool(_re.match(r"\d{4}-\d{2}-\d{2}\.(csv|jsonl)$", p.name))
    files = (
        sorted(td.glob("*_tokens.csv")) +
        sorted(td.glob("*_tokens.jsonl")) +
        sorted(f for f in td.glob("*.csv")   if _is_date_file(f) and "_tokens" not in f.name) +
        sorted(f for f in td.glob("*.jsonl") if _is_date_file(f) and "_tokens" not in f.name)
    )
    # 날짜 기준 중복 제거 (tokens 버전 우선)
    seen_dates = set()
    deduped = []
    ...
    # top_tokens: frequency 기준으로 선택 (is_meaningful 미적용)
    # Z-score가 균일 단어(노이즈)를 자동 제거 → correlation에 유리
    # dashboard용 필터는 아래 top_by_z에서 별도 적용
    top_tokens = [
        t for t, _ in sorted(totals.items(), key=lambda x: x[1], reverse=True)
        if len(t) >= args.min_len
    ][: args.topk * 3]  # 더 많이 뽑아서 Z-score로 걸러냄
    ...
    # dashboard 표시용: is_meaningful로 노이즈 단어 제거
    top_by_z = sorted(
        [t for t in top_tokens if is_meaningful(t, args.min_len)],
        key=lambda t: zscores[t], reverse=True
    )
    # correlation용 terms: is_meaningful 미적용 (Z-score 필터로 충분)
    top_tokens = top_tokens[: args.topk]  # 원래 크기로 복원
